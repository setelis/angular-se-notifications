...
.provider("SeNotificationsService", function SeNotificationsServiceProvider() {
	"use strict";
	var provider = this;

	/*jshint -W072 */
	function SeNotificationsService($log, $timeout, $window, $rootScope, $injector, SeVersionsService, options) {
		/*jshint +W072 */
		var service = this;
		var lastStates = [];
		...
		function attachMethods() {
			...
			service.handleHttpError = function(handlersConfiguration) {
				...
				return function(errorResponse) {
					...
					if (!handled) {
						service.notificationBuilder("httperrors.unknown", null, service.httpErrorToDebugInfo(errorResponse), false)
			...
			service.showNotificationInfo = function(msgKey, parameters, tag, debugInfo) {
				tag = tag || msgKey;
				service.notificationBuilder(msgKey, parameters, debugInfo, false).type(service.TYPE.TEXT)
	...
	/*jshint -W072 */
	provider.$get = ["$log", "$timeout", "$window", "$rootScope", "$injector", "SeVersionsService",
		function SeNotificationsServiceFactory($log, $timeout, $window, $rootScope, $injector, SeVersionsService) {
		/*jshint +W072 */
		var effectiveOptions = _.assign({}, DEFAULT_OPTIONS, customizedOptions);

		return new SeNotificationsService($log, $timeout, $window, $rootScope, $injector, SeVersionsService, effectiveOptions);
	}];
